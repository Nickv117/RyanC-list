...
router.get("/category/:slug", (req, res, next) => {
  ...
  const sql = `SELECT p.id, p.name, p.posting, p.time_created FROM posts p LEFT JOIN categories c ON p.catergory_id = c.id WHERE c.slug =? ORDER BY p.time_created DESC LIMIT 50`
